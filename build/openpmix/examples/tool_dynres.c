...
char pmix_server_nspace[]="prterun-dominik-VirtualBox-717004@0,0";
...
int main(int argc, char **argv)
{
    ...
    for (n = 1; n < (size_t) argc; n++) {
        if (0 == strcmp("-u", argv[n]) || 0 == strcmp("--url", argv[n])) {
            ...
            ++n;
            ninfo += 2;
        } else if (0 == strcmp("-pid", argv[n]) || 0 == strcmp("--pid", argv[n])) {
            /* retrieve the PMIx server's uri from the indicated node */
    ...
    if (NULL != nspace){
        PMIX_INFO_LOAD(&info[n], PMIX_SERVER_NSPACE, nspace, PMIX_STRING);
        ++n;
    }
    if (0 != pid){
        PMIX_INFO_LOAD(&info[n], PMIX_SERVER_PIDINFO, &pid, PMIX_INT);
    }
    ...
    char rc_cmd[PMIX_MAX_KEYLEN];

    do{

        printf("Enter a resource change command or type exit\n");
        if(NULL == fgets(rc_cmd, PMIX_MAX_KEYLEN, stdin))continue;
        rc_cmd[strlen(rc_cmd)-1]='\0';
        printf("sending command '%s' to server\n",rc_cmd);
        pmix_info_t *info;
        PMIX_INFO_CREATE(info,2);
        strcpy(info[0].key, "PMIX_RC_CMD");
        PMIX_VALUE_LOAD(&info[0].value, (void**)&rc_cmd, PMIX_STRING);
        (void)snprintf(info[1].key, PMIX_MAX_KEYLEN, "%s", PMIX_EVENT_NON_DEFAULT);
        bool non_default=true;
        PMIX_VALUE_LOAD(&info[1].value, &non_default, PMIX_BOOL);
        printf("PMIX_VALUE.string is %s\n", info[0].value.data.string);

        PMIx_Notify_event(PMIX_RC_DEFINE, &myproc, PMIX_RANGE_SESSION, info, 2, NULL, NULL);
        PMIX_INFO_FREE(info,2);

    }while(strcmp(rc_cmd,"exit")!=0);
