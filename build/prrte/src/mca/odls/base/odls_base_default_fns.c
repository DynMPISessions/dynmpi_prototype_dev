...
int prte_odls_base_default_update_pmix_server_data(pmix_data_buffer_t *buffer)
{
    ...
    for (n = 0; n < jdata->procs->size; n++) {
        ...
        if (prte_get_attribute(&jdata->attributes, PRTE_JOB_FULLY_DESCRIBED, NULL, PMIX_BOOL)) {
            ...
            printf("new node\n");
            //if not already in map, create a copy of the daemon node so we do not manipulate the original
            int j;
            prte_node_t *node_cpy = NULL;
            bool node_found = false;
            for(j = 0; j < jdata->map->nodes->size; j++){
                if(NULL == (node_cpy = prte_pointer_array_get_item(jdata->map->nodes, j))){
                    continue;
                }
                if(strcmp(node_cpy->name, dmn->node->name)){
                    node_found = true;
                    break;
                }
            }
            printf("searched\n");
            if(!node_found){
                node_cpy = PRTE_NEW(prte_node_t);
                memcpy(node_cpy, dmn->node, sizeof(prte_node_t));
                node_cpy->procs = PRTE_NEW(prte_pointer_array_t);
                
                /* add the node to the job map */
                PRTE_RETAIN(node_cpy);
                prte_pointer_array_add(jdata->map->nodes, pptr->node);
                jdata->map->num_nodes++;
                PRTE_FLAG_SET(node_cpy, PRTE_NODE_FLAG_MAPPED);
            }
...
next:
    ...
    for (n = 0; n < jdata->procs->size; n++) {
        ...
        /* see if it belongs to us */
        if (pptr->parent == PRTE_PROC_MY_NAME->rank) {
            printf("%s: Proc with app rank %d, has local rank %d. %d slots in use on node\n", PRTE_NAME_PRINT(PRTE_PROC_MY_NAME), pptr->app_rank, pptr->local_rank, pptr->node->slots_inuse);
            /* is this child on our current list of children */
            if (!PRTE_FLAG_TEST(pptr, PRTE_PROC_FLAG_LOCAL)) {
                /* not on the local list */
                PRTE_OUTPUT_VERBOSE((5, prte_odls_base_framework.framework_output,
