...
int prte_set_or_replace_job_data_object(prte_job_t *jdata)
{
    ...
    if(-1 < exists){
        jdata->index = exists;
        jptr = (prte_job_t *) prte_pointer_array_get_item(prte_job_data, exists);
        jptr->index = -1;
        prte_pointer_array_set_item(prte_job_data, exists, jdata);
        PRTE_RELEASE(jptr);
        jptr = (prte_job_t *) prte_pointer_array_get_item(prte_job_data, exists);
    }
    else if (-1 == save) {
...
prte_proc_t *prte_get_proc_object_by_rank(const prte_job_t *jdata, pmix_rank_t rank)
{
    ...
    for(pr = 0 ; pr < jdata->procs->size; pr++){
        if(NULL == (proct = prte_pointer_array_get_item(jdata->procs, pr))){
            continue;
        }
...
pmix_rank_t prte_get_proc_daemon_vpid(const pmix_proc_t *proc)
{
    ...
    if (NULL == (proct = (prte_proc_t *) prte_get_proc_object(proc))) {
        return PMIX_RANK_INVALID;
    }
