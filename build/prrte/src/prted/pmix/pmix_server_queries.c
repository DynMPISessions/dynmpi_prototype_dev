...
static void _query(int sd, short args, void *cbdata)
{
    ...
    if (PMIX_SUCCESS == ret) {
        if (0 == prte_list_get_size(&results)) {
            ...
            {
                //if(0 == strcmp(kv->info.key, PMIX_QUERY_PSET_MEMBERSHIP)){
                //    printf("query_membership: num_results=%d\n", rcd->ninfo);
                //}
                //if(0 == strcmp(kv->info.key, "PMIX_RC_PSET")){
                //    printf("query_rc: num_results=%d\n", rcd->ninfo);
                //}
