...
pmix_status_t update_job_data_sub(prte_res_change_t *res_change){
    ...
    for(p = 0; p < rc_pset->num_members; p++){
        ...
        for(a = 0; a < jdata->map->nodes->size; a++){
            ...
            /* update node */
            printf("%s: Node slots before finalize: inuse: %d, total: %d, procs: %d\n", PRTE_NAME_PRINT(PRTE_PROC_MY_NAME), node->slots_inuse, node->slots, node->num_procs);
            for(n = 0; n < node->procs->size; n++){
                ...
                if(PMIX_CHECK_PROCID(&rc_proc, &job_proc->name)){
                    printf("removing proc with local rank %d\n", job_proc->local_rank);
            ...
            printf("%s: Node slots after finalize: inuse: %d, total: %d, procs: %d\n", PRTE_NAME_PRINT(PRTE_PROC_MY_NAME), node->slots_inuse, node->slots, node->num_procs);
