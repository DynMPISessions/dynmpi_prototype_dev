/**
 * @file examples/p4est_mpidynres.cpp
 * 
...
MPI_Info create_info_from_state(simulation_state *state) {
    ...
    if(strcmp(state->scenario, "") != 0) {
        MPI_Info_set(info, "scenario", state->scenario);
    } else {
        MPI_Info_set(info, "bathymetry", state->bathymetry);
        MPI_Info_set(info, "displacement", state->displacement);
    }
...
bool check_resource_change(simulation_state *state) {
    ...
    int working_size = state->p4est->mpisize;
    int idle_size = -1;
    ...
    if(mpirank == 0) {
        ...
        int optimal_num_cores = 
            state->p4est->global_num_quadrants / 10000 + 1;
        int delta = optimal_num_cores - state->p4est->mpisize;
 ...
 * Initializes the MPIDYNRES_Session environment, the p4est, the scenario,
...
simulation_state initialize(int argc, char* argv[]) {
    ...
    if(contains_key) {
        ...
        args.addOption("scenario", 's', 
                ...
                args.Required, false);
        args.addOption("bathymetry-path", 'b', 
                "Load bathymetry input from a file, overwrites -s option", 
                args.Required, false);
        args.addOption("displacement-path", 'd', 
                "Load displacement data from a file, to be used in\
                    combination with -b", 
                args.Required, false);
        ...
        if(!args.isSet("bathymetry-path") && 
                !args.isSet("displacement-path")) {
            scenario_option = 
                args.getArgument<std::string>("scenario", "dambreak");
        } else if(args.isSet("bathymetry-path") && 
                args.isSet("displacement-path")) {
            bathymetry_path = 
                args.getArgument<std::string>("bathymetry-path");
            displacement_path = 
                args.getArgument<std::string>("displacement-path");
        } else {
            cout << "The options -b and -d must be used together.\n";
            MPIDYNRES_exit();
        }
        ...
        if(exists) {
            strcpy(state.bathymetry, "");
            strcpy(state.displacement, "");
        } else {
            int disp_exists;
            MPI_Info_get(session_info, "bathymetry", 99, state.bathymetry, &exists);
            MPI_Info_get(session_info, "displacement", 99, 
                    state.displacement, &disp_exists);
            if(exists && disp_exists) {
                strcpy(state.scenario, "");
            } else {
                strcpy(state.bathymetry, "");
                strcpy(state.displacement, "");
                strcpy(state.scenario, "dambreak");
            }
        }
    }
...
#ifdef NETCDF
        scenario = new SWE_TsunamiScenario(state.bathymetry, 
                state.displacement, state.t_end,
...
/**
 * @brief free all allocated structures and destroy the p4est
 */
...
int simulate(simulation_state *state){
    ...
    for(; state->current_phase <= state->num_phases && 
        ...
        if(changed) {
            if(state->is_new)
                state->p4est = NULL;
            state->p4est = p4est_dynres_replace(state->p4est, 
                    state->mpicomm);
        }
        ...
        l_writer->writeTimeStep(state->p4est);
        std::cout << "\t" << state->current_phase << ": " << state->p4est->global_num_quadrants << "\n";
