/**
 * @file benchmarks/p4est_benchmark_constProcs_split.cpp
 * 
 * Benchmark for growing number of quadrants on a constant number
 * of processes. The grid is refined uniformly at every phase, 
 * so rebalancing and repartitioning will have only a small influence here.
 * The total number of quadrants starts at 4^6 = 4096, and is quadrupled 
 * at every phase to max. 4^11 = 4194304 quadrants.
 * The number of processes is 56 on two nodes.
 * To be simulated with 10 phases (refinement is done at every 
 * second phase).
 */
...
int simulate(simulation_state *state){
    ...
    if(state->p4est->mpirank == 0) {
        output_file.open("constProcs_split_log.csv");
        output_file << 
