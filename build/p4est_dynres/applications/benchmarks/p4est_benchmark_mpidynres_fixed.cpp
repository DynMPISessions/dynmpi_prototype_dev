...
float simulate_interval(simulation_state *state) {
    ...
    do {
        // reset updates
        printf("_____________________1\n");
        p4est_iterate (p4est, NULL,
                NULL,
                resetUpdates,
                NULL,
                NULL);
        printf("__________________2\n");
...
int simulate(simulation_state *state){
    ...
    if(rank == 0) {
        output_file.open("mpidynres_fixed_log.csv");
        output_file << 
