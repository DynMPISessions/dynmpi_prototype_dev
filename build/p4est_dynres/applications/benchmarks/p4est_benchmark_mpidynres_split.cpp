/**
 * @file benchmarks/p4est_benchmark_mpidynres_split.cpp
 * 
 * Benchmark for p4est on libmpidynres using an increasing 
 * number of quadrants. 
 * Number of quadrants going from 4^6 to 4^11, quadrupled at avery 
 * second phase
 * Number of processes going from 45 to 55, adding one process at every phase
 * Start with 56 processes on two nodes, as libmpidynres needs its own process
 * Use 10 phases
 * 
 */
...
int simulate(simulation_state *state){
    ...
    if(rank == 0) {
        output_file.open("mpidynres_split_log.csv");
        output_file << 
