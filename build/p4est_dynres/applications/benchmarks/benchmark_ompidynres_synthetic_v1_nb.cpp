...
int resource_change_step(MPI_Session *session_handle, MPI_Comm *lib_comm, char *pset_name, char *delta_pset, char *pset_result, int *terminate, MPI_Request *request){
    ...
    if(! pend_flag){
        ...
        if(my_rank==0){
            ...
            rc = MPI_Session_dyn_recv_res_change(*session_handle, pset_name, &rc_type, delta_pset, &incl_flag);
        ...
        if(my_rank == 0 && rc_type != MPI_RC_NULL){ 
        ...
        MPI_Bcast(delta_pset, 511, MPI_CHAR, 0 , *lib_comm);


        /*********************** integrate resource change *************************/

        *terminate = 0;
        make_timestamp_root(&cur_synth_timing_frame->accept_start);
        if(my_rank == 0){
            rc = MPI_Session_dyn_integrate_res_change_nb(session_handle, info, delta_pset, pset_result, 1, terminate, request);
        }else{
            rc = MPI_Session_dyn_integrate_res_change_nb(session_handle, info, delta_pset, pset_result, 0, terminate, request);
        }
    ...
    /* Non-blocking: MPI_Test */
    //int test_flag = 0; 
    //rc = MPI_Test(request, &test_flag, &req_status);
    //pend_flag = test_flag == 1 ? 0 : 1;
    
    /* Blocking: MPI_Wait */
    rc = MPI_Wait(request, &req_status);
    

    if(rc == MPI_SUCCESS && pend_flag){
        rc = MPI_ERR_PENDING;
    }


    /* if successful create new communicator, else need to break (here indicated by MPI_COMM_NULL)*/
    if(MPI_SUCCESS == rc && pend_flag == 0){
        if(MPI_REQUEST_NULL != *request){
            MPI_Request_free(request);
        }
        
        MPI_Comm_disconnect(lib_comm);
...
int main(int argc, char* argv[])
{
    ...
    int incl_flag = 0, terminate_flag = 0;
    //system("rm -R /dev/shm/*");
    ...
    int ret = MPI_Session_dyn_recv_res_change(session_handle, MPI_SELF_PSET, &rc_type, delta_pset, &incl_flag);
    ...
    if(rc_type != MPI_RC_NULL && incl_flag){
        ...
        rc = MPI_Session_dyn_integrate_res_change(&session_handle, info, delta_pset, pset_name, 0, &terminate_flag);

        make_timestamp_base(&cur_base_timing_frame->confirm_end);
        
        if(MPI_SUCCESS == rc){
            printf("    DELTA PSET PROC: Confirmation succeeded. Communication will be initiated via pset: %s\n", pset_name);
        }else{
            printf("    DELTA PSET PROC: Confirmation failed. ERROR: %d\n", rc);
        }
    ...
    while(counter++< ITER_MAX){
        ...
        if(check_rc && iter_since_change++ >= rc_frequency){
            ...
            rc = resource_change_step(&session_handle, &lib_comm, pset_name, delta_pset, pset_result, &terminate_flag, &request);
