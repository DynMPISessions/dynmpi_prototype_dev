/**
 * @file benchmarks/p4est_benchmark_constProcs_fixed.cpp
 * 
 * @brief benchmark for the simulation an a p4est 
 * 
 * Benchmark for a constant number of quadrants on a constant number
 * of processes. 
 * The total number of quadrants is 4^10 = 1048576,
 * the number of processes is 56 on two nodes.
 *
 * Simulate with 10 phases for comparability.
 */
...
struct simulation_state {
    ...
    std::string output_name;        // the name of the output file written to
...
int simulate(simulation_state *state){
    ...
    if(state->p4est->mpirank == 0) {
        output_file.open("constProcs_fixed_log.csv");
        output_file << 
