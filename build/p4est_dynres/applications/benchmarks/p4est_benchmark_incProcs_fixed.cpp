/**
 * @file benchmarks/p4est_benchmark_incProcs_fixed.cpp
 * 
 * Benchmark for constant number of quadrants on a growing number
 * of processes.
 * Number of quadrants: 4^10
 * To be started with 56 processes on two nodes,
 * will use 46 to 56 cores (add one at each phase.)
 * Run with 10 phases.
 */
...
int simulate(simulation_state *state){
    ...
    if(rank == 0) {
        output_file.open("dynProcs_fixed_log.csv");
        output_file << 
            "Checkpoint,Quadrants,Proccesses,Iterations,TotalTime,refinementTime,partitionTime,dynresTime\n";
    }
