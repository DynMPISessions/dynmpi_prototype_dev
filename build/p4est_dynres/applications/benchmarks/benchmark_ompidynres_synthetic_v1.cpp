...
int resource_change_step(MPI_Session *session_handle, MPI_Comm *lib_comm, char *pset_name, char *delta_pset, char *pset_result, int *terminate){
    ...
    *terminate = 0;
    ...
    if(my_rank == 0){
        rc = MPI_Session_dyn_integrate_res_change(session_handle, info, delta_pset, pset_result, 1, terminate);
    }else{
        rc = MPI_Session_dyn_integrate_res_change(session_handle, info, delta_pset, pset_result, 0, terminate);
    }
