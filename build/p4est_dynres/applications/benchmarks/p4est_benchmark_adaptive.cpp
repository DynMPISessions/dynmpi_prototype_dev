/**
 * @file benchmarks/p4est_benchmark_adaptive.cpp
 * 
 * Benchmark for a fully adaptive simulation: resolution and
 * number of processes dependent of workload
 * Using from 4^7 to 4^15 quadrants, starting at 4^9,
 * and at most 55 processes (at least 10000 quadrants per process),
 * starting at 14 processes.
 * Start with 56 processes, as libmpidynres needs its own process,
 * and use 10 phases for comparability.
 *
 * Takes a *very* long time, for demonstration use the mpidynres2d
 * example instead.
 * 
 */
...
float simulate_interval(simulation_state *state) {
    ...
    do {
        ...
        iterations++;
        printf("__________5: %f of %f \n", t, state->phases[state->current_phase]);
...
int simulate(simulation_state *state){
    ...
    if(rank == 0) {
        output_file.open("adaptive_log.csv");
        output_file << 
