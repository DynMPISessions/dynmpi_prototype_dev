...
int get_comm(MPIDYNRES_Session mysession, char *name, MPI_Comm *mycomm) {
  MPI_Group mygroup;
  int err;

  printf("Get comm %s\n", name);

  err = MPIDYNRES_Group_from_session_pset(mysession, name, &mygroup);
  if (err) {
    printf("Something went wrong\n");
    return err;
  }
  printf("Got group, now creating comm\n");
  err = MPIDYNRES_Comm_create_from_group(mygroup, NULL, MPI_INFO_NULL,
...
bool resource_changes_check(struct run_state *state) {
  ...
  switch (rc_type) {
    ...
    case MPIDYNRES_RC_SUB: {
      ...
      MPIDYNRES_Session_get_psets(state->session, MPI_INFO_NULL, &psets_info);
...
int MPIDYNRES_main(int argc, char *argv[]) {
  ...
  MPIDYNRES_Session_get_info(state->session, &state->info);
  ...
  MPIDYNRES_Session_finalize(&state->session);
