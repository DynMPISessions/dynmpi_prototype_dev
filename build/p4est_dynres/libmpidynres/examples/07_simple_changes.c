...
MPIDYNRES_Session session;  // session object
...
void update_main_comm() {
  MPI_Group mygroup;
  MPIDYNRES_Group_from_session_pset(session, main_pset, &mygroup);
  MPIDYNRES_Comm_create_from_group(mygroup, NULL, MPI_INFO_NULL, MPI_ERRORS_ARE_FATAL,
                             &main_comm);
...
void handle_resource_changes(MPIDYNRES_RC_type rc_type, char delta_pset[],
  ...
  switch (rc_type) {
    ...
    case MPIDYNRES_RC_SUB: {
      ...
      MPIDYNRES_Session_get_psets(session, MPI_INFO_NULL, &mypsets);
...
void initialization_phase() {
  ...
  MPIDYNRES_Session_init(MPI_INFO_NULL, MPI_ERRORS_ARE_FATAL, &session);

  MPIDYNRES_Session_get_psets(session, MPI_INFO_NULL, &psets);
  ...
  if (info_contains(psets, "mpi://WORLD")) {
    ...
    MPIDYNRES_Session_get_info(session, &session_info);
...
void finalization_phase() {
  ...
  printf("comm freed\n");
  MPIDYNRES_Session_finalize(&session);
  printf("finalized\n");
}
...
int MPIDYNRES_main(int argc, char *argv[]) {
  ...
  for (; main_iter < 10 && !need_to_break; main_iter++) {
    work_step();
