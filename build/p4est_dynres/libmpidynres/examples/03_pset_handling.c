...
int print_pset_info(MPIDYNRES_Session mysession) {
  int err, num_psets;
  MPI_Info info, psets_info;
  char key[MPI_MAX_INFO_KEY + 1];

  // Get all available process sets
  err = MPIDYNRES_Session_get_psets(mysession, MPI_INFO_NULL, &psets_info);
  if (err) {
  ...
  for (int i = 0; i < num_psets; i++) {
    ...
    err = MPIDYNRES_Session_get_pset_info(mysession, key, &info);
    if (err) {
      printf("Something went wrong\n");
      return err;
    }
...
int print_session_info(MPIDYNRES_Session mysession) {
  int err;
  MPI_Info info;
...
int main(int argc, char *argv[static argc + 1]) {
  ...
  printf("MPIDYNRES_SIM_start finished\n");
