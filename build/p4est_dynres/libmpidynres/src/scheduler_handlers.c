...
/**
 * @brief      Handle a resource change requested by the user
 *
 * @details    Query management interface for delta pset and insert it to rc set
 * and send it to cr
 *
...
void DEBUG_MPIDYNRES_scheduler_handle_user_rc(MPIDYNRES_scheduler *scheduler,
  ...
  // if pending, shutdowns, return failure
  if (scheduler->pending_shutdowns.size > 0) {
    debug("Warning: RC was requested, but there are still pending shutdowns\n");
    rc_type = MPIDYNRES_RC_NONE;
    info = MPI_INFO_NULL;
  } else {
    err = DEBUG_MPIDYNRES_manager_handle_user_rc_msg(scheduler->manager,
            cr_id, delta, &info, &new_pset);
    if (delta > 0) {
