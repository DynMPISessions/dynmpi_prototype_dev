 ...
 *
 * @param      dest The rank of the sending process
 *
 * @param      comm The communicator used
 *
 * @return     On error, a value != 0 is returned
 */
int MPIDYNRES_Bcast_Send_MPI_Info(MPI_Info info, int root, MPI_Comm comm) {
  int res;
  int nkeys;
  int vlen;
  int unused;
  ...
  if (info == MPI_INFO_NULL) {
    ...
    res = MPI_Bcast(&bufsize, 1, my_MPI_SIZE_T, root, comm);
    if (res) {
      return res;
    }
    return 0;
  ...
  res = MPI_Bcast(buffer, bufsize, MPI_BYTE, root, comm);
  free(buffer);
  if (res) {
    return res;
  }
 ...
 * @return     On error, a value != 0 is returned
 */
int MPIDYNRES_Bcast_Recv_MPI_Info(MPI_Info *info, int root,
                            MPI_Comm comm) {
  int res;
  size_t bufsize;
  struct info_serialized *serialized;
  ...
  if (bufsize == SIZE_MAX) {
    printf("Info Bcast recv: info was null\n");
    *info = MPI_INFO_NULL;
    return 0;
  }
  ...
  res = MPI_Bcast(buf, bufsize, MPI_BYTE, root, comm);
  if (res) {
    free(buf);
    return res;
  }
  ...
  for (size_t i = 0; i < serialized->num_strings / 2; i++) {
    ...
    //printf("Bcast_Recv_Info: received %s:%s\n", key, val);
    if (res) {
      free(buf);
      return res;
    }
