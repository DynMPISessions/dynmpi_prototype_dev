...
void MPIDYNRES_scheduler_schedule(MPIDYNRES_scheduler *scheduler) {
  ...
  MPI_Irecv(&user_rc_msg, 1, MPI_INT, MPI_ANY_SOURCE, MPIDYNRES_TAG_USER_RC,
            scheduler->config->base_communicator, &requests[user_rc_request]);
  ...
  while (scheduler->running_crs.size > 0) {
    ...
    switch (index) {
      ...
      case user_rc_request: {
        DEBUG_MPIDYNRES_scheduler_handle_user_rc(scheduler, &status, session_id, user_rc_msg);
        printf("MPIDYNRES user request received with delta %d\n", user_rc_msg);
