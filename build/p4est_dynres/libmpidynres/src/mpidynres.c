...
MPIDYNRES_Session MPIDYNRES_Session_NULL = NULL;
 ...
 *
 * @details    The MPIDYNRES_Session_init function is used to create a new
...
int MPIDYNRES_Session_init(MPI_Info info, MPI_Errhandler errhandler,
  ...
  int unused = 0;
  int err;
  debug("In MPIDYNRES_Session_init\n");

  debug("allocating internal mpi session with %zu bytes\n",
        sizeof(struct internal_MPIDYNRES_Session));

  struct internal_MPIDYNRES_Session *sess =
      calloc(1, sizeof(struct internal_MPIDYNRES_Session));
  ...
  if (sess->session_id == MPIDYNRES_INVALID_SESSION_ID) {
    *session = MPIDYNRES_Session_NULL;
 ...
 *
 * @details    The MPIDYNRES_Session_finalize is used to destruct an MPI session and
 * has to be called on each constructed session object before a process exits.
 * It frees the session argument and replaces it with MPIDYNRES_Session_NULL. The
...
int MPIDYNRES_Session_finalize(MPIDYNRES_Session *session) {
  int err;
  int answer;
  debug("In MPIDYNRES_Session_finalize\n");
  if (*session == MPIDYNRES_Session_NULL) {
    debug("Warning: MPIDYNRES_Session_finalize called with MPIDYNRES_Session_NULL\n");
    return 0;
  }
 ...
 *
 * @details    MPIDYNRES_Session_get_info is used to obtain hints associated with the
 * session argument.
 ...
 *
 * @details    The MPIDYNRES_Session_get_psets function is used to query process sets
...
int MPIDYNRES_Session_get_psets(MPIDYNRES_Session session, MPI_Info info, MPI_Info *psets) {
  int err;
  if (session == MPIDYNRES_Session_NULL) {
    debug("Warning: MPIDYNRES_Session_get_psets called with MPIDYNRES_Session_NULL\n");
    return 1;
  }
...
int MPIDYNRES_Session_get_pset_info(MPIDYNRES_Session session, char const *pset_name,
  ...
  if (session == MPIDYNRES_Session_NULL) {
    debug("Warning: MPIDYNRES_Session_get_pset_info called with MPIDYNRES_Session_NULL\n");
    return 1;
  }
  if (pset_name == NULL) {
    debug("Warning: MPIDYNRES_Session_get_pset_info called with NULL name\n");
    return 1;
  }
 ...
 * @return     if !=0, an error occured
 */
int MPIDYNRES_Group_from_session_pset(MPIDYNRES_Session session, char const *pset_name,
                                MPI_Group *newgroup) {
  int err;
  int msg[2];
  size_t answer_size;
 ...
 * @return     if != 0, an error occured
 */
int MPIDYNRES_Comm_create_from_group(MPI_Group group, const char *stringtag,
 ...
 * @return     if != 0, an error occured
 */
int MPIDYNRES_pset_create_op(MPIDYNRES_Session session, MPI_Info hints,
 ...
 * @return     if != 0, an error occured
 */
int MPIDYNRES_pset_free(MPIDYNRES_Session session,
 ...
 * @return     if != 0, an error occured
 */
int MPIDYNRES_add_scheduling_hints(MPIDYNRES_Session session, MPI_Info hints,
                                   MPI_Info *answer) {
  int err;
...
/**
 * @brief request a resource change from the user program.
 *
 * Requests a specific change to the used resources in order to
 * deal with reuirements from the user application. Used for debugging
 * purposes only.
 *
 * @param [in] session the MPI session
 * @param [in] rc_type the type of the requested RC
 * @param [in] delta the number of resources in the delta_pset
 * @param [out] delta_pset the resources changed after the request
 * @param [out] tag the RC tag
 * @param [out] info an MPI_Info object with more information
 */
int DEBUG_MPIDYNRES_RC_user_request(MPIDYNRES_Session session, 
  ...
  err = MPI_Send((void*) &delta, 1, MPI_INT, 0, MPIDYNRES_TAG_USER_RC,
                 g_MPIDYNRES_base_comm);
  if (err) {
    return err;
  }
 ...
 * @return     if != 0, an error occured
 */
int MPIDYNRES_RC_get(MPIDYNRES_Session session, MPIDYNRES_RC_type *rc_type,
                     char delta_pset[MPI_MAX_PSET_NAME_LEN],
                     MPIDYNRES_RC_tag *tag, MPI_Info *info) {
  int err;
  MPIDYNRES_RC_msg answer = {0};
